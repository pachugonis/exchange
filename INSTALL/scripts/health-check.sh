...
check_containers() {
    ...
    # Check optional containers (nginx may not be running if SSL not configured)
    for container in "${optional_containers[@]}"; do
        if docker ps --format '{{.Names}}' | grep -q "^${container}$"; then
            print_success "Container ${container}: Running"
        else
            print_warning "Container ${container}: Not running (may need SSL configuration)"
        fi
    done
...
check_http() {
    ...
    while [ $attempt -le $max_attempts ]; do
        if curl --max-time 3 -s -o /dev/null -w "%{http_code}" http://localhost:80 2>/dev/null | grep -q "200\|301\|302"; then
    ...
    print_warning "HTTP: Not responding (check nginx logs)"
    return 0
}
...
check_https() {
    ...
    while [ $attempt -le $max_attempts ]; do
        if curl --max-time 3 -k -s -o /dev/null -w "%{http_code}" https://localhost:443 2>/dev/null | grep -q "200\|301\|302"; then
    ...
    print_warning "HTTPS: Not responding (check nginx logs)"
    return 0
}
...
main() {
    ...
    if [[ $failed -eq 0 ]]; then
        print_success "вњ“ All critical health checks passed!"
        echo ""
        print_info "Your application is ready at: https://${DOMAIN}"
    else
        print_warning "вљ  Some health checks failed (${failed} issues found)"
